...
Author: Debbie Rios
Date: 07/04/2013
Modified:   07/22/2013   by: Debbie Rios
Comments: Add test
...
class TestPeterNorvigAlgorithm(unittest.TestCase):
    ...
    def setUp(self):
        ...
        grid2 = ''
        self.sudoku2 = PeterNorvigAlgorithm(grid2)
        self.level1 = Level(20, 25, '')

    def testCrossMethodReturnSquareOf81Characters(self):
       square = self.sudoku.crossProduct(self.rows, self.cols)
    ...
    def testIfGridValuesMethodReturnGridMapaAsDic(self):
        grid = '0030206009003050010018064000081029007000000080067082000026095\
                00800203009005010300'
        sudoku = self.sudoku.getGridValues(grid)
    ...
    def testIfgetDifficultLevelMethodReturnAInteger(self):
        expectedResults = self.sudoku2.getDifficultLevel(self.level1)
        self.assertEqual(expectedResults.isdigit(), False)

    def testIfRandomPuzzleMethodGenerateASudoku(self):
        level = 25
        expectedResults = self.sudoku2.randomPuzzle(level)
        self.assertEqual(expectedResults.isdigit(), False)
